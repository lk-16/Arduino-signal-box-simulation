...
public:
    ...
    int nextWay(int knotenNr, int fahrstrassenNr_vorgeaenger = 0, boolean vorgaengerAktiv = false);       /**<Die Methode gibt die Nr des nächsten nicht markierten Nachbarn (oder ein markierter Nachbar, welcher einen Weg beseitzt, nützt suchenOpt. Diese Kombi taucht nur in diesem Fall der Suche auf) der angegebenen Knotennummer zurück, wenn angegeben, den Knoten, der mit der FahrstrassenNr versehen ist. WEnn vorgaenger Aktiv, wird statt nach einer Fahrstraße, der Vorgänger des aktuellen Knotens übergeben, so kann ein logisch Sinnvoller WEg zum Beispiel bei WEichen bestimmt werden.*/
    ...
    int wegSuchenOpt(Gleissymbol *start, Gleissymbol * ziel, Gleissymbol * vorgaenger = nullptr, int laenge = 0);         /**<Die Methode ermittelt den Optimalen Weg, sollten mehr als zwei Wege existieren*/
